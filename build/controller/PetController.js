...
function checkPetType(specie, res) {
    if (!Object.values(enumSpecies_1.default).includes(specie)) {
        res.status(500).json({ error: "Invalid Specie!" });
    }
}
function checkPetSize(size, res) {
    if (size && !Object.values(enumSizes_1.default).includes(size)) {
        return res.status(500).json({ error: "Invalid size!" }).end();
    }
}
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (size && !Object.values(enumSizes_1.default).includes(size)) {
                            return [2 /*return*/, res.status(500).json({ error: "Invalid size!" }).end()];
                        }
                        newPet = new Pet_1.default(name, specie, bornDate, adopted, size);
                        return [4 /*yield*/, petRepository.create(newPet)];
                    case 1:
                        _b.sent();
                        return [2 /*return*/, res.status(201).json(newPet)];
                }
            });
        });
    };
    ...
    PetController.prototype.findPetsBySize = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                ...
                return [2 /*return*/, res.status(200).json(req.query)];
            });
        });
    };
